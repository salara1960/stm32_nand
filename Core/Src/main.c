...
const char *eol = "\r\n";
const char *s_cmds[MAX_CMDS] = {"restart", "epoch:", "read:", "next", "write:", "erase:", "check:", "area:", "save:"};
uint8_t devError;
uint8_t cmd_flag = 0;
...
static uint32_t epoch = 1653602199;//1653563627;
uint8_t tZone = 0;//2;
		...
		if ((qStat = osMessageQueueGet(myQueHandle, (void *)&qcmd, &prio, 10)) != osOK) {
			...
			switch (qcmd.cmd) {
				...
				case cmdErase:
					if (!qcmd.attr) {
						NAND_AddressTypeDef addr = {
							.Page = 0,//nand_BlockToPage(nandBlk),
							.Plane = 1,
							.Block = nandBlk
						};
